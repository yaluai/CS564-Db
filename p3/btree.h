...
class BTreeIndex {
 ...
 public:
  ...
  const PageId splitLeafNode(LeafNodeInt *leftNode, int& key, const RecordId rid);

  const PageId splitNonLeafNode(NonLeafNodeInt* node, int &intKey, const PageId pageId);

  const bool insertKeyIntoLeafNode(LeafNodeInt *node, int key, const RecordId rid);

  const bool insertKeyIntoNonLeafNode(NonLeafNodeInt* node, int key, PageId pageId);

  const inline void initNonLeafNode(NonLeafNodeInt* node, int start = 0, int end = INTARRAYNONLEAFSIZE);

  const inline void initLeafNode(LeafNodeInt* node, int start = 0, int end = INTARRAYLEAFSIZE);

  const int getScanStartNodeForNextEntry(PageId pageNum);

  const inline void BufferUnPinPage(const PageId pageNo, const bool dirty);
